...
trait BinarySerializer[T] { self =>
  ...
  final def serialize(value: T, output: BinaryOutput): Either[DesertFailure, Unit] =
    serialize(value).run(output)
...
object BinarySerializer {
  type Ser[T] = ReaderT[Either[DesertFailure, *], BinaryOutput, T]

  object Ser {
    final def fromEither[T](value: Either[DesertFailure, T]): Ser[T] = ReaderT.liftF(value)
  }
}

trait BinarySerializerOps {
  final def getOutput: Ser[BinaryOutput] = ReaderT.ask[Either[DesertFailure, *], BinaryOutput]
  ...
  final def write[U : BinarySerializer](value: U): Ser[Unit] = getOutput.flatMap(output => Ser.fromEither(output.write(value)))
...
trait BinaryDeserializer[T] { self =>
  ...
  final def deserialize(input: BinaryInput): Either[DesertFailure, T] =
    deserialize().run(input)
...
object BinaryDeserializer {
  type Deser[T] = ReaderT[Either[DesertFailure, *], BinaryInput, T]

  object Deser {
    final def fromEither[T](value: Either[DesertFailure, T]): Deser[T] = ReaderT.liftF(value)
  }
}
