...
trait LowerPriorityGenericDerivationApi {
  implicit def hlistSerializer[K <: Symbol, H, T <: HList](implicit
      ...
      tailCodec: ChunkedBinarySerializer[T]
  ): ChunkedBinarySerializer[FieldType[K, H] :: T]

  implicit def hlistDeserializer[K <: Symbol, H, T <: HList](implicit
      witness: Witness.Aux[K],
      headCodec: Lazy[BinaryCodec[H]],
      tailCodec: ChunkedBinaryDeserializer[T]
  ): ChunkedBinaryDeserializer[FieldType[K, H] :: T]
...
/** Trait containing the public interface for the generic binary codec.
  ...
  * [[ChunkedBinaryDeserializer]]. These monads are more specific than the [[BinarySerializer]] and
  * [[BinaryDeserializer]], holding state specific for a single evolvable value's serialization.
  */
trait GenericDerivationApi extends LowerPriorityGenericDerivationApi {
  ...
  implicit def hlistTransientSerializer[K <: Symbol, H, T <: HList](implicit
      witness: Witness.Aux[K],
      tailCodec: ChunkedBinarySerializer[T]
  ): ChunkedBinarySerializer[FieldType[K, MarkedAsTransient[H]] :: T]
  ...
  implicit def hlistOptionalDeserializer[K <: Symbol, H, T <: HList](implicit
      ...
      tailCodec: ChunkedBinaryDeserializer[T]
  ): ChunkedBinaryDeserializer[FieldType[K, Option[H]] :: T]

  implicit def hlistTransientDeserializer[K <: Symbol, H, T <: HList](implicit
      witness: Witness.Aux[K],
      tailCodec: ChunkedBinaryDeserializer[T]
  ): ChunkedBinaryDeserializer[FieldType[K, MarkedAsTransient[H]] :: T]
  ...
  /** Type class to tag a labelled generic representation of a type with a type level information about whether each
    ...
    * implementation of the [[ChunkedBinaryDeserializer]] and [[ChunkedBinarySerializer]] type classes.
    *
    * @tparam H
  ...
  /** The entry point of deriving a generic binary codec for a type 'T'
    ...
    * and the deserializer through the implicits of [[ChunkedBinarySerializer]] and [[ChunkedBinaryDeserializer]].
    *
...
class GenericBinaryCodec(evolutionSteps: Vector[Evolution]) extends GenericDerivationApi {
  ...
  implicit def hlistTransientSerializer[K <: Symbol, H, T <: HList](implicit
      witness: Witness.Aux[K],
      tailCodec: ChunkedBinarySerializer[T]
  ): ChunkedBinarySerializer[FieldType[K, MarkedAsTransient[H]] :: T] = { case _ :: tailValues =>
  ...
  implicit def hlistSerializer[K <: Symbol, H, T <: HList](implicit
      ...
      headCodec: Lazy[BinaryCodec[H]],
      tailCodec: ChunkedBinarySerializer[T]
  ): ChunkedBinarySerializer[FieldType[K, H] :: T] = { case headValue :: tailValues =>
    for {
  ...
  private def readOptionalFieldIfExists[H](
      fieldName: String
  )(implicit headCodec: Lazy[BinaryCodec[H]], optHeadCodec: Lazy[BinaryCodec[Option[H]]]): ChunkedDeser[Option[H]] =
  ...
  private def readFieldIfExists[H](fieldName: String)(implicit headCodec: Lazy[BinaryCodec[H]]): ChunkedDeser[H] =
    ChunkedDeserOps.getChunkedInput.flatMap { chunkedInput =>
      ChunkedDeserOps.getChunkedState.flatMap { chunkedState =>
    ...
    () =>
      for {
        ...
        headValue    <- chunkedState.transientFields.get(Symbol(fieldName)) match {
                          ...
                          case None        =>
                            ChunkedDeserOps.failWith(
                              DeserializationFailure(s"Illegal state while processing transient field $fieldName", None)
                            )
                        }
    ...
    () =>
      for {
        ...
        result          <- if (witness.value.name == constructorName) {
                             ChunkedDeserOps
                               .fromDeser(headCodec.value.deserialize(), input)
                               .map(headValue => Inl(field[K](headValue)))
                           } else {
    ...
    BinaryCodec.define[T] { value =>
      for {
        ...
        finalState    <- Ser.fromEither(
                           serializer.value
                             .serialize(taggedTransients.tag(genericValue))
