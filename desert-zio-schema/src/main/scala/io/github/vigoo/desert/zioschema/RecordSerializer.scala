...
private[zioschema] object RecordSerializer {
  ...
  implicit def caseClass3Serializer[A1, A2, A3, Z]: RecordSerializer[Schema.CaseClass3[A1, A2, A3, Z]] =
    (schema: Schema.CaseClass3[A1, A2, A3, Z], value: Any) =>
      List(
        ...
        AdtCodec.SerializationCommand.WriteField(
          ...
          () => DerivedBinaryCodec.derive(schema.field3.schema.asInstanceOf[Schema[Any]])
        )
      )
}
